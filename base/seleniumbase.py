...
class SeleniumBase:
    ...
    def __get_selenium_by(self, find_by: str) -> dict:  # __ to get private (invisible) in child classes
    ...
    def is_visible(self, find_by: str, locator: str, locator_name: str = None) -> WebElement:    # the element is visible on the page
        return self.__wait.until(ec.visibility_of_element_located((self.__get_selenium_by(find_by), locator)), locator_name)

    def is_present(self, find_by: str, locator: str, locator_name: str = None) -> WebElement:   # the element is present on the page
        return self.__wait.until(ec.presence_of_element_located((self.__get_selenium_by(find_by), locator)), locator_name)

    def is_not_present(self, find_by: str, locator: str, locator_name: str = None) -> WebElement:   # the element is not present
        return self.__wait.until(ec.invisibility_of_element_located((self.__get_selenium_by(find_by), locator)), locator_name)

    def are_visible(self, find_by: str, locator: str, locator_name: str = None) -> List[WebElement]:   # multi elements are visible
